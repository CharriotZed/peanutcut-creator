...
2. **联网研究**——统一走内置的 `scripts/web_search.py`（qianfan web search 网关）查受众画像、爆款案例、竞品格局、变现路径、平台算法动向（**不直接用 Claude Code/Codex 自带的 WebSearch**，保证搜索源可控、结果结构统一），拿到 url 后按需用 WebFetch 抓正文；同时读取内置的 `references/platforms/*.md` 平台算法拆解作为补充和校验。
