...
- **抓取网页正文**：Claude Code 用 `WebFetch`；Codex 用其网页读取能力。拿到 `web_search.py` 返回的 url 后，若需要读全文再用它抓取。
...
**联网研究（用 `scripts/web_search.py` 网关搜索，别用自带 WebSearch；拿到 url 后按需用 WebFetch 抓正文）**，围绕选定的平台和垂类，至少覆盖：
