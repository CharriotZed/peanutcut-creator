...
这是一个"生成 skill 的 skill"。它不直接帮你写选题、写文稿、做封面——它做的是把这一整套能力，按照你的平台、垂类和人设，量身定制成一个**独立、可安装、开箱即用**的专属工作流 skill，覆盖从选题到文稿、标题、封面的完整链路，并把最终成片收口到**花生 AI**（素材剪辑 + MG 动画 + 本地素材，口播驱动）。你只需要回答几个最基础的问题（还可以选择性地给一个想参考的对标账号），剩下的受众画像、爆款打法、变现路径、平台算法动向，它会自己联网研究补齐，先给你一份账号定位诊断和策略提案确认方向，确认后再落地生成产物。目标是让每个账号拿到的都不是一份通用模板，而是一套真正吃透了自己平台算法和垂类打法、且能直接落到花生 AI 成片的作战手册。

## 最终成片工具：花生 AI

**这套工作流的终点是一条能直接用花生 AI 做出来的视频。** 花生 AI 的成片方式是「素材剪辑 + MG 动画 + 部分本地素材」，由一段口播文稿驱动整条视频——它最适合**文本口播驱动**的视频。因此本技能所有产出（选题、文稿、标题、封面）都要向花生的能力收口：

- **选题**：优先选口播讲得动、画面素材找得到的题（详见选题方法论"可执行性"门槛）。
- **文稿**：只交一段干净的纯口播稿，画面/动画由花生按语义自动生成，稿子里不写任何分镜或画面提示。
- **标题/封面**：负责让做出来的视频在平台被点开，和成片是并行的两条线。

生成产物时，务必把这条"最终走花生 AI"的约束写进产物各模块正文。完整逻辑见 [花生 AI 成片方法论](references/methodology/peanut-production.md)。

## 运行环境与脚本路径

本技能面向 **Claude Code**（也兼容其它遵循 SKILL.md 规范的 agent）。文中的工具名按 Claude Code 的能力理解：

- **联网搜索/抓取网页**：用 `WebSearch` / `WebFetch`。若当前环境确实无联网能力，如实告知用户无法获取实时数据，退回内部知识作研究依据，不要用记忆里的旧数据冒充实时情报。
- **写文件**：用 `Write` 工具。生成产物 skill 时涉及的每一个文件（SKILL.md、各模块文档、脚本、配置）都要真实落地到磁盘，不能只在对话里描述。
- **跑脚本**：通过 shell 执行 `python3`。脚本路径见下方——一律用技能目录的绝对路径，不要假设当前工作目录就是技能目录。

本技能自身的脚本（结构校验器 `validate_skill.py` 等）随技能一起安装，位于技能目录 `~/.claude/skills/video-workflow-builder/scripts/`。**不要假设 shell 的当前工作目录是技能目录**（运行时 cwd 通常是用户项目目录）。运行脚本前先定位技能目录，用绝对路径调用：
...
下文示例为简洁起见写成 `scripts/xxx.py`，实际调用时请替换为 `"$SKILL_DIR/scripts/xxx.py"`。生成的产物 skill 安装后同理有自己的技能目录（`~/.claude/skills/<账号名>-workflow`），产物内部脚本调用时也按这个规则解析绝对路径——这一点会写入产物自身的 SKILL.md。
...
- [文稿方法论](references/methodology/script-writing.md) — 跨平台/跨垂类文稿判断逻辑（Hook结构、节奏控制与信息密度、完播设计、互动引导、兑现标题承诺），**只输出纯口播稿、中文标点规范**，时长与竖屏/横屏形态由 `references/platforms/*.md` 在生成时注入。
...
- [花生 AI 成片方法论](references/methodology/peanut-production.md) — 工作流最后一环：花生 AI 靠素材剪辑 + MG 动画 + 本地素材、由口播稿驱动成片。解释为什么文稿只交纯口播稿、怎么让口播语义可被花生自动配上画面、本地素材怎么配合。所有产物都要把"最终走花生"写进正文。
...
用户发起请求后，**只问这三件必答 + 一件选答**：

1. **平台**（可多选，从抖音/B站/小红书/视频号/百家号里选一个或多个）
2. **垂类**（做什么内容方向，如财经、职场、历史、美食、科技测评……）
3. **人设**（账号以什么身份/口吻出镜，如"资深投资人""过来人前辈""专业测评师"）
4. **（选答）对标账号**：问一句"有没有想参考/模仿的目标账号？有的话给我账号名或主页链接，没有也没关系。"——如果用户能给出一个对标账号，阶段1 就能通过拆解它更快、更准地搭起工作流，并在此基础上给出差异化方案；用户答不上来或不想给，直接跳过，不要追问。

**明确不要问**受众画像、差异化定位、变现路径——这些属于研究阶段和诊断阶段的产出，不是访谈阶段该问用户的。用户往往自己也说不清楚"目标受众是谁""怎么和同类账号差异化"，把这些问题甩给用户只会让访谈变得又长又低效。必答三件 + 选答一件问完、拿到答案就立刻进入阶段1，不要追加更多访谈问题。

若用户在一句话里已经把信息都说全了（例如"我想在抖音做美食测评账号，人设是社区大厨，想参考'XX厨房'"），直接确认理解无误后进入阶段1，不必再走一轮问答。
...
**对标账号拆解（若阶段0拿到了对标账号）**：把用户给的对标账号作为研究的加速器和锚点，用联网工具查这个账号的公开信息，拆出可复用的规律：

- **选题规律**：它常做哪几类选题、选题池怎么分布、命中的主要是哪种动机（恐惧/愤怒/希望/好奇……）
- **标题/封面套路**：标题的常用句式与钩子类型、封面的固定排布与配色风格
- **文稿与节奏**：口播的开头钩子怎么设计、时长区间、信息密度、互动引导方式
- **人设与差异化空间**：它立住的是什么人设、评论区受众在夸什么/骂什么、还有哪些角度它没占满

拆完之后**不是照抄**，而是提炼出"值得复用的成熟打法"+"可以差异化的空档"两份清单，作为阶段2 差异化定位的直接输入。差异化方案要具体到"对标账号做A，我们改做B"，而不是泛泛说"做得更好"。若对标账号信息查不到或过少，如实说明，退回纯垂类研究，不要编造它的数据。

**若无法联网**：如实告知用户当前环境没有联网能力，明确说明本次研究将只依赖内部知识和已内置的平台资料，不会有实时热点和最新数据；绝不用训练记忆里的旧案例冒充"当下爆款"或编造搜索结果。

各条线的信息汇总后，才能进入阶段2出诊断——研究不到位，诊断就是空话。
...
- **差异化定位**：这个账号相比已有竞品，打的是哪个没被占满的角度或人群。若阶段1 拆解过对标账号，这里要具体到"对标账号做A，我们改做B"的差异点，而不是泛泛说"做得更好"
...
- **各模块独立调用方式**：选题、文稿、标题、封面四个模块除了走完整流程，也可以单独唤起（比如用户已经有文稿只想起标题），逐一说明怎么单独触发
- **怎么落到花生 AI 成片**：明确告诉用户——文稿模块产出的是一段纯口播稿，**直接粘进花生 AI 即可成片**，画面、MG 动画由花生按口播语义自动生成，不需要用户再写分镜；若有必须用到的本地素材（实拍片段、独家截图、数据原图），在花生成片时一起导入
...
**注入花生 AI 成片约束**：产物的文稿模块必须写清"只输出纯口播稿、中文标点规范、不带任何画面/分镜标注、语义画面友好"，并把"最终粘进花生 AI 成片、画面与 MG 动画由花生自动生成、本地素材另行导入"作为交付习惯写进产物正文（参考 [花生 AI 成片方法论](references/methodology/peanut-production.md) 与文稿模板 `script-writing.md.tmpl`）。选题模块的可执行性门槛也要包含"口播讲得动、素材找得到"这条。这些约束要**改写进产物正文**，不能只留一个指回本生成器的链接。
...
- 产物的文稿模块必须约束**只输出纯口播稿**：无章节标题、无画面/镜头/分镜提示、无秒数标注，且全程使用中文标点符号（不混用英文标点）——因为最终要粘进花生 AI 成片，画面与 MG 动画由花生按口播语义自动生成。
- 产物的选题模块可执行性门槛必须包含"口播讲得动、画面素材找得到"这条花生 AI 适配要求。
