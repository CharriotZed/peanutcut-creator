...
**Goal:** 编写一个单文件 Python 脚本 `scrape_douyin.py`，使用 Playwright 抓取抖音用户主页的账号数据、全部视频的指标与评论，并生成结构化 JSON + Markdown 分析报告。

**Architecture:** 单文件脚本 `scripts/scrape_douyin.py`，依赖 `playwright`。独立运行，不耦合项目现有 content_db 模块。非 headless Chromium + 手动扫码登录，顺序抓取，每视频立即刷盘（断点续抓）。

**Tech Stack:** Python 3, Playwright (Chromium), stdlib (json/os/re/sys/time/argparse/datetime)

## Global Constraints

- 脚本文件：`video-workflow-builder/scripts/scrape_douyin.py`
- 依赖新增：`pip install playwright && playwright install chromium`
- 非 headless 模式运行
- 单线程顺序抓取，间隔 1-3 秒
- 每视频即时 flush 写 JSON（断点可恢复）
- 评论 Top 20 条/视频
- 输出目录：脚本同级的 `data/` 目录
- 编码风格对齐项目现有脚本（stdlib 优先，argparse CLI，`#!/usr/bin/env python3`）

---

### Task 1: 脚本骨架 + CLI 参数 + 数据模型

**Files:**
- Create: `video-workflow-builder/scripts/scrape_douyin.py`

**Interfaces:**
- Produces: `main()` entry, `parse_args()`, `DATA_DIR` resolution, data model type hints
...
```bash
python3 video-workflow-builder/scripts/scrape_douyin.py --url https://www.douyin.com/user/test
```
Expected: 打印"数据输出目录: .../data"，创建 data/ 目录，退出码 0。

---

### Task 2: 浏览器启动 + 扫码登录 + session 持久化

**Files:**
- Modify: `video-workflow-builder/scripts/scrape_douyin.py` — 追加函数

**Interfaces:**
- Produces: `launch_browser()` → `(browser, context, page)`, `login_if_needed(page, timeout, session_path)` → None, `save_session(context, session_path)` → None

- [ ] **Step 1: 追加浏览器启动和登录函数**

在 `scrape_douyin.py` 的 `resolve_data_dir` 之后、`main` 之前追加以下代码：

```python
def launch_browser(session_path=""):
    """启动非 headless Chromium，返回 (browser, context, page)。"""
...
```bash
python3 video-workflow-builder/scripts/scrape_douyin.py --url https://www.douyin.com/user/MS4w... --timeout 120
```
Expected: 浏览器打开抖音首页，显示登录提示；扫码后可检测到登录状态并保存 session。
...
**Files:**
- Modify: `video-workflow-builder/scripts/scrape_douyin.py` — 追加 `scrape_account(page, url)` 函数

**Interfaces:**
- Produces: `scrape_account(page, url)` → `dict`  (账号数据)
- Consumes: 已登录的 Playwright `page` 对象

- [ ] **Step 1: 追加账号抓取函数**

在 `save_session` 之后、`main` 之前追加：
...
- [ ] **Step 2: 更新 main 函数进行集成测试**

将 Task 2 的 main 中 `input("按 Enter...")` 之后改为调用 `scrape_account`：

在 main 的 try 块中，`input(...)` 之后追加：
...
- [ ] **Step 3: 试跑验证**（手动扫码，观察终端输出）

```bash
python3 video-workflow-builder/scripts/scrape_douyin.py --url https://www.douyin.com/user/MS4w...
```
Expected: 扫码后，终端打印出解析到的昵称、粉丝、获赞、作品数。

---

### Task 4: 作品列表滚动加载 + 视频 URL 收集

**Files:**
- Modify: `video-workflow-builder/scripts/scrape_douyin.py` — 追加 `collect_video_links(page, expected_count)` 函数

**Interfaces:**
- Produces: `collect_video_links(page, expected_count)` → `list[str]` (视频详情页 URL 列表)
- Consumes: 已在主页上的 `page`
...
- [ ] **Step 2: 更新 main 函数加入视频收集**

在 Task 3 的 main 中账号抓取之后、写入 JSON 之前，加入：
...
**Files:**
- Modify: `video-workflow-builder/scripts/scrape_douyin.py` — 追加 `scrape_video(page, video_url)` 函数

**Interfaces:**
- Produces: `scrape_video(page, video_url)` → `dict` (视频数据，不含评论)
- Consumes: `page`, `video_url`
...
**Files:**
- Modify: `video-workflow-builder/scripts/scrape_douyin.py` — 追加 `scrape_comments(page, max_count)` 函数

**Interfaces:**
- Produces: `scrape_comments(page, max_count)` → `list[dict]`
- Consumes: 已在视频详情页的 `page`
...
**Files:**
- Modify: `video-workflow-builder/scripts/scrape_douyin.py` — 追加 `generate_report(account, videos, output_path)` 函数

**Interfaces:**
- Produces: `generate_report(account, videos, output_path)` → `str` (报告文件路径)
- Consumes: 账号 dict + 视频 list + 输出路径
...
**Files:**
- Modify: `video-workflow-builder/scripts/scrape_douyin.py` — 更新 `main()` 函数，串联全部流程

**Interfaces:**
- Consumes: Task 1-7 的所有函数
- Produces: 完整的端到端抓取流程

- [ ] **Step 1: 用完整 main 函数替换当前的 main**
...
# 运行完整流程
python3 video-workflow-builder/scripts/scrape_douyin.py \
  --url https://www.douyin.com/user/MS4wLjABAAAAtz8b29P1BD5Mq8lVZMEL7n_gK5zxH7_R8jRPR_j2O00 \
  --max-comments 20
```

Expected: 浏览器打开 → 扫码 → 自动抓取账号数据 → 滚动收集视频 → 逐视频抓取指标+评论 → 输出 JSON + MD 报告。
...
**Files:**
- Modify: `video-workflow-builder/scripts/scrape_douyin.py`

在 `scrape_videos_batch` 中的 `scrape_video` 调用处加上 try/except，确保单个视频失败不中断整体流程。

- [ ] **Step 1: 加固视频抓取的异常处理**

将 `scrape_videos_batch` 中的：
...
将 `scrape_comments` 整个函数体用 try/except 包裹，失败时返回空列表。

- [ ] **Step 3: 验证健壮性**

模拟网络中断或页面加载失败，确认脚本能继续处理剩余视频。

```bash
# 杀掉浏览器进程模拟崩溃，重新运行检查断点续抓是否生效
python3 video-workflow-builder/scripts/scrape_douyin.py --url <URL> --max-comments 20
```
